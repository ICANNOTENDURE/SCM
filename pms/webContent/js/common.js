...
function includeJS(filePath,force){
	...
	if('undefined'!=typeof(force) && force) INCLUDE_JS_MAP[reURL]=false;
	if('undefined'!=typeof(INCLUDE_JS_MAP[reURL]) 
			&& INCLUDE_JS_MAP[reURL]){
		return;
	}
	jQuery.ajax({
		async: false,
...
function includeCSS(filePath, force){
	...
	if('undefined'!=typeof(force) && force) INCLUDE_CSS_MAP[reURL]=false;
	...
	jQuery.ajax({
		...
		scriptCharset: "utf-8",
		success: function(d){
			if(/['"]?([_\-a-zA-Z0-9\.]+)\.(png|jpg|jpeg|gif)['"]?/i.test(d)){
				var xdp=filePath.substr(0, filePath.lastIndexOf('/'));
				d = d.replace(/['"]?([_\/\\\-a-zA-Z0-9\.]+)\.(png|jpg|jpeg|gif|bmp)['"]?/ig,
				'"'+$WEB_ROOT_PATH+xdp+"/$1.$2\"");
			}
			var styleTag = '<style media="screen" type="text/css" styleID="'+_sid+'">'+d+'</style>';
...
/**
 * 自适应iframe高度
 * @param iframe 可以是：iframe元素的id | jQuery选择器 | dom对象 | jQuery对象
 */
